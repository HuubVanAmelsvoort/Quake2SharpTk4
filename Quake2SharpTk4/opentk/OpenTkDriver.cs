...
public abstract class OpenTkDriver : OpenTkGL, GLDriver {
  ...
  public int setMode(Size dim, int mode, bool fullscreen) {
    ...
    HuubZetWindow(fullscreen, windowSize);
  ...
  private void HuubZetWindow(bool fullscreen, Size windowSize) {
    NativeWindowSettings nativeSettings = new NativeWindowSettings();
    nativeSettings.Title = "HuubQuake2Sharp";
    nativeSettings.API = ContextAPI.OpenGL;
    nativeSettings.APIVersion = new Version(3, 2);
    nativeSettings.Profile = ContextProfile.Compatability;
    nativeSettings.Flags = ContextFlags.Default;

    if (fullscreen) {
      var displayMode =
      this.getModeList().FirstOrDefault(
      displayMode => displayMode.Width == windowSize.Width && displayMode.Height == windowSize.Height)
      ?? this.oldDisplayMode;
      nativeSettings.WindowState = WindowState.Fullscreen;
      VID.Printf(
      Defines.PRINT_ALL,
          $"...volledig scherm {displayMode.Width}x{displayMode.Height}x{displayMode.BitDepth}@{displayMode.RefreshRate}Hz\n"
        );
    } else {
      nativeSettings.Size = new Vector2i(windowSize.Width, windowSize.Height);
    }

    try {
      this.window = new GameWindow(GameWindowSettings.Default, nativeSettings);
    } catch (Exception e) {
      VID.Printf(Defines.PRINT_ALL, $"Fout tijdens window init: {e.Message} einde verhaal\n");
      Environment.Exit(-3);
    }
